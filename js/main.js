...
function setupD3Visualization() {
    // Asegurarse de que el SVG esté limpio antes de dibujar para evitar duplicados.
    d3.select("#network-chart").selectAll("*").remove();

    // Data extraída del NotebookLM y resumen
    const graphData = {
        "nodes": [
...
// El resto del código de D3.js sigue aquí sin cambios...
...
function setupVisualizationModal() {
    ...
    if (!openBtn) return; // Si no está el botón principal, no hacemos nada.

    let isVizInitialized = false;

    function openViz() {
        modalOverlay.classList.remove('hidden');
        // Solo inicializa la visualización la primera vez que se abre el modal.
        if (!isVizInitialized) {

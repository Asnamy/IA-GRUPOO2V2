...
function setupVisualizationModal() {
    ...
    const svgContainer = document.getElementById('network-chart');

    if (!openBtn) return; // Si no hay botón, no hacer nada

    function openViz() {
        modalOverlay.classList.remove('hidden');
        // Es CRUCIAL llamar a la función de D3 *después* de que el modal sea visible,
        // para que pueda calcular las dimensiones correctamente.
        setupD3Visualization(); 
    }

    function closeViz() {
        modalOverlay.classList.add('hidden');
        // Limpiamos el contenido del SVG para evitar duplicados si se vuelve a abrir.
        if (svgContainer) {
